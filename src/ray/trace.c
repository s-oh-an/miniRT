...
int	is_ray_hit_object(t_object_list *obs, t_ray *ray, t_color *color)
{
	...
	while (obs)
	{
		...
		{
			*color = ((t_sphere *)obj->property)->color;
			return (is_ray_hit_sphere((t_sphere *)obj->property, ray));
		}
		else if (obj->type == T_PLANE)
		{
			*color = ((t_plane *)obj->property)->color;
			return (is_ray_hit_plane((t_plane *)obj->property, ray));
		}	
		else if (obj->type == T_CYLINDER)
		{
			(*color) = ((t_cylinder *)obj->property)->color;
			return (is_ray_hit_cylinder((t_cylinder *)obj->property, ray));
		}
		else
			return (0);
		obs = obs->next;
